...
def _voucher_pdf_bytes(voucher: dict[str, Any], *, branding: dict[str, Any] | None = None) -> bytes:
    ...
    left = 48
    right = page_w - 48
    y = draw_society_pdf_header(pdf, title="Accounting Voucher", branding=branding, margin_x=left)
    ...
    pdf.drawString(left, y, _pdf_text(f"Generated by {society_name} / MitraBooks accounting")[:110])
    pdf.save()
...
async def transaction_voucher_pdf(
    ...
    branding = await get_housing_society_branding(tenant_id=tenant_id, app_key=app_key)
    pdf_bytes = _voucher_pdf_bytes(voucher, branding=branding)
