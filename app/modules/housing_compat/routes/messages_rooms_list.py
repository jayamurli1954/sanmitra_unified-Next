...
async def messages_list_rooms(
    ...
    await housing_router._get_or_create_message_room(
        tenant_id=tenant_id,
        app_key=app_key,
        name="General",
        room_type="general",
    ...
    for row in rows:
        if await housing_router._can_access_message_room(room=row, current_user=current_user, tenant_id=tenant_id, app_key=app_key):
            visible_rooms.append(housing_router._room_response(row))
