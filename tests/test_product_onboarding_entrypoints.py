...
def test_mitrabooks_public_entry_redirects_folder_root_to_landing() -> None:
    ...
    manifest_source = (REPO_ROOT / "frontend" / "mitrabooks-erp" / "manifest.webmanifest").read_text(
        encoding="utf-8"
    )
    local_server = (REPO_ROOT / "scripts" / "serve_frontends.py").read_text(encoding="utf-8")

    assert "publishMitraBooksLandingIndex" not in build_script
    assert "fs.copyFileSync(appShell, loginShell)" not in build_script
    assert "fs.copyFileSync(landingShell, appShell)" not in build_script
    assert '"/mitrabooks-erp/"' in vercel_config
    assert '"/mitrabooks-erp/landing.html"' in vercel_config
    assert '"/mitrabooks-erp/login"' in vercel_config
    assert '"/mitrabooks-erp/login.html"' in vercel_config
    assert 'path == "/mitrabooks-erp"' in local_server
    assert 'Location", "/mitrabooks-erp/landing.html"' in local_server
    assert "./login.html" in landing_source
    assert "./login.html" in onboarding_source
    assert 'id="login-form"' in index_source
    assert '"start_url": "./index.html"' in manifest_source
