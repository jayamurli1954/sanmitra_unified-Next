...
Architecture authority: `docs/architecture/OFFICEMITRA_AI_IMPLEMENTATION_PLAN.md` and `docs/adr/ADR-001` … `ADR-017`. Review workspace PRD: `docs/prd/OFFICEMITRA_REVIEW_WORKSPACES.md`. Documents package PRD: `docs/prd/OFFICEMITRA_DOCUMENTS_PACKAGE.md`.

### Current vs target (discipline)

- Current: OfficeMitra AI Phase 1 (tasks/email/brief) plus Phase 2 productivity (paste-in calendar, meeting notes, in-app notifications), Phase 3 live read connectors, Phase 5 standalone shell, Phase 4 confirmed OfficeMitra-owned write-back behind `office_ai.writeback` (ADR-008), Phase 6 workflow engine behind `office_ai.workflows` (ADR-009), ADR-012 Policy Engine (shared confirm/approve/execute decisions), and [ADR-014](docs/adr/ADR-014-officemitra-ca-analysis-pack.md) CA Analysis Pack (Accepted — implementation gated by `office_ai.mis*` flags) under module `office_ai` with modular deployment (ADR-007). [ADR-015](docs/adr/ADR-015-officemitra-review-workspaces.md) Review / working papers / review notes is implemented behind `office_ai.review*` (**default off**). [ADR-016](docs/adr/ADR-016-officemitra-documents-package.md) Documents package is implemented behind `office_ai.documents` (**default off**; staff CA queue via connector). [ADR-017](docs/adr/ADR-017-officemitra-missing-document-requests.md) staff missing-document requests are implemented behind `office_ai.documents.requests` (**default off**; OfficeMitra task only, no client email).
- Target: Productivity depth without OAuth mailboxes/calendars; optional connectors enrich the brief when companion modules are enabled; action-based proposals for owned writes; multi-step owned workflows reuse the Action Executor; one policy API for UI/executor/workflows; three OfficeMitra Review workspaces (scan, working papers, notes) behind `office_ai.review*` — no new Mitra product names; staff Documents package behind `office_ai.documents` linking Review notes to the MitraBooks CA queue, plus staff missing-document tasks behind `office_ai.documents.requests`.
- Deferred: Gmail/Outlook/Google Calendar OAuth, multi-model routing marketplace, WhatsApp/agents/vector DB, companion-product write-back (journals/GST/legal/housing) until [ADR-010](docs/adr/ADR-010-officemitra-companion-writeback.md) is Accepted, InvestMitra. CA Analysis Pack / MIS ([ADR-014](docs/adr/ADR-014-officemitra-ca-analysis-pack.md) Accepted) ships behind `office_ai.mis*` flags — template Excel import first; no journal/GST writes from MIS flows. Client portal, client missing-doc emails, compliance calendar, knowledge, notices, advisory, team, and analytics remain later OfficeMitra packages (Documents staff-queue linking is ADR-016; staff task requests are ADR-017; portal is still deferred), never new Mitra brands. Domain events / template library remain [ADR-011](docs/adr/ADR-011-officemitra-domain-events.md) / [ADR-013](docs/adr/ADR-013-officemitra-workflow-template-library.md).
...
- Gate routes on module `office_ai` plus sub-feature flags `office_ai.tasks`, `office_ai.email`, `office_ai.brief`, `office_ai.calendar`, `office_ai.meeting_notes`, `office_ai.notifications`, opt-in `office_ai.writeback` / `office_ai.workflows` (ADR-008/009; default off), opt-in `office_ai.mis` + import/export/`live_mitrabooks`/pack flags (ADR-014; default off), opt-in `office_ai.review` / `office_ai.review.working_papers` / `office_ai.review.notes` (ADR-015; default off), opt-in `office_ai.documents` (ADR-016; default off), and opt-in `office_ai.documents.requests` (ADR-017; default off; requires parent Documents).
- Do not add new Mitra product brands. Review, working papers, and review notes are OfficeMitra workspaces ([ADR-015](docs/adr/ADR-015-officemitra-review-workspaces.md)). Staff Documents is an OfficeMitra package ([ADR-016](docs/adr/ADR-016-officemitra-documents-package.md)) that reuses the MitraBooks CA queue via a connector. Later practice capabilities are OfficeMitra packages, not brands.
- Documents (ADR-016 / ADR-017): list CA staff documents through the MitraBooks connector; store `ca_document_id` on Review notes only. Staff missing-document requests create OfficeMitra tasks (optional Review note); they do not email the client. Do not query `business_ca_document_metadata` from OfficeMitra code. Do not create a client portal. Do not upload or change CA status from OfficeMitra until ADR-010 is Accepted.
- CA Analysis Pack (ADR-014): MIS facts immutable after reconcile/export; template Excel import v1; live MitraBooks MIS reads behind `office_ai.mis.live_mitrabooks` (report services only, `app_key=mitrabooks`); narrative requires fact citations; no journal/invoice/GST writes from MIS flows; PPT export defaults to maker-checker.
- Review ingest (ADR-015): SSDV is internal; do not post uploaded books into the live MitraBooks PostgreSQL ledger. Review risk score must not be aliased to MIS `data_quality_score`.
...
| 1.8 | 2026-09-12 | OfficeMitra Review workspaces (ADR-015): no new Mitra brands; Review / working papers / review notes only; later practice capabilities are OfficeMitra packages |
| 1.9 | 2026-09-12 | OfficeMitra Documents package (ADR-016): staff CA queue via connector; Review note links; no client portal |
| 1.10 | 2026-09-12 | OfficeMitra staff missing-document requests (ADR-017): gap report + OfficeMitra task; no client email/portal |
...
**Bootstrap other repos:** `docs/operations/GRAPHIFY_BOOTSTRAP.md` (User Rule + copy `docs/templates/cursor-graphify.mdc`).
