...
Architecture authority: `docs/architecture/OFFICEMITRA_AI_IMPLEMENTATION_PLAN.md` and `docs/adr/ADR-001` … `ADR-016`. Review workspace PRD: `docs/prd/OFFICEMITRA_REVIEW_WORKSPACES.md`. Documents package PRD: `docs/prd/OFFICEMITRA_DOCUMENTS_PACKAGE.md`.
...
- Documents (ADR-016): list CA staff documents through the MitraBooks connector; store `ca_document_id` on Review notes only. Do not query `business_ca_document_metadata` from OfficeMitra code. Do not create a client portal. Do not upload or change CA status from OfficeMitra until ADR-010 is Accepted.
