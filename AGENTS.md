...
- Gate routes on module `office_ai` plus sub-feature flags `office_ai.tasks`, `office_ai.email`, `office_ai.brief`, `office_ai.calendar`, `office_ai.meeting_notes`, `office_ai.notifications`, opt-in `office_ai.writeback` / `office_ai.workflows` (ADR-008/009; default off), and opt-in `office_ai.mis` + import/export/pack flags (ADR-014; default off).
