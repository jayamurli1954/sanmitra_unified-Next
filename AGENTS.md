...
- Gate routes on module `office_ai` plus sub-feature flags `office_ai.tasks`, `office_ai.email`, `office_ai.brief`, `office_ai.calendar`, `office_ai.meeting_notes`, `office_ai.notifications`, opt-in `office_ai.writeback` / `office_ai.workflows` (ADR-008/009; default off), opt-in `office_ai.mis` + import/export/pack flags (ADR-014; default off), and opt-in `office_ai.review` / `office_ai.review.working_papers` / `office_ai.review.notes` (ADR-015; default off).
- Do not add new Mitra product brands. Review, working papers, and review notes are OfficeMitra workspaces ([ADR-015](docs/adr/ADR-015-officemitra-review-workspaces.md)). Later practice capabilities are OfficeMitra packages, not brands.
