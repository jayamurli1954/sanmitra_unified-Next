...
function syncPasteFields(el) {
  ...
  syncReviewFields(root, state); syncDocumentsFields(root, state);
}
...
export function renderOfficeAiWorkspace() {
  ...
  if (state.reviewEnabled) tabs.splice(featureInsertAt++, 0, ["review", "Review"]); if (state.documentsEnabled) tabs.splice(featureInsertAt++, 0, ["documents", "Documents"]);
  ...
  else if (state.tab === "review") body = renderReviewPanel(state, escapeHtml); else if (state.tab === "documents") body = renderDocumentsPanel(state, escapeHtml);
      ...
      <div class="preview-heading compact">
        <div>
          ...
          ${state.misEnabled
            ...
            : ""}
          ${reviewBannerHtml(state)}${documentsBannerHtml(state)}
...
export async function loadOfficeAiWorkspace() {
  ...
  try {
    ...
    else if (state.tab === "review") await refreshReviewData(state, { apiRequest, unwrap }); else if (state.tab === "documents") await refreshDocumentsData(state, { apiRequest, unwrap });
