...
| 2026-05-22 | MandirMitra demo public payment submission | Demo-only UX added | `/mandir-public/` can create pending public payments only for demo/test tenants; default local demo uses `seed-tenant-1` so the known admin login can verify it | Verify the pending demo payment through ERP Public Payments and confirm receipt/accounting posting |
