...
**Current engineering status (2026-08-16):** Stage 2 research trust bar is met for the GST §54 / IT §139 slices (response contract, answer feedback, Stage 2 eval ≥95% grounding, local/staging statute corpus ops). Senior-counsel answer spine is live for §54. Stage 2.1 Quality Gate + statute Citation Audit are shipped (mismatch fault eval ≥95%). Engineering exit: [`docs/operations/LEGALMITRA_STAGE2_EXIT_SIGNOFF.md`](../operations/LEGALMITRA_STAGE2_EXIT_SIGNOFF.md). **Deferred from Stage 2/2.1:** case-law citators, full Bare Act expansion. **Partial progress (2026-08-16):** starter curated judgment retrieve-or-refuse for GST §54 “referred cases” (VKC Footsteps digest). Commercial adoption gate to Stage 3 remains parallel.

**Stage 2.1 (complete for engineering exit 2026-08-16):** Named Quality Gate + statute-first Citation Audit on the hybrid research path; fault fixtures + UI “Sources checked.” Remaining deferred: broader claim segmentation, case-law citators. Spec: [`docs/architecture/LEGALMITRA_STAGE2_1_QUALITY_GATE_AND_CITATION_AUDIT.md`](../architecture/LEGALMITRA_STAGE2_1_QUALITY_GATE_AND_CITATION_AUDIT.md).
...
**Current engineering status (2026-08-16):** Stage 5 Prepare Matter Response is hardened behind `LEGALMITRA_AGENTIC_ENABLED` (off by default in production): RESEARCH uses the Stage 2 hybrid contract, human approve/reject/retry/cancel, Morning Brief CTA, HTTP isolation tests. Spec: [`docs/architecture/LEGALMITRA_STAGE5_AGENTIC_WORKFLOWS.md`](../architecture/LEGALMITRA_STAGE5_AGENTIC_WORKFLOWS.md). Sign-off: [`docs/operations/LEGALMITRA_STAGE5_HARDENING_SIGNOFF.md`](../operations/LEGALMITRA_STAGE5_HARDENING_SIGNOFF.md). KG precision claim and automatic judgment retrieval for “referred cases” remain deferred.
