...
| 2026-05-22 | MandirMitra demo public payment submission | Demo-only UX added | `/mandir-public/` can submit a pending public payment only when the selected tenant is marked demo/test; live trusts remain visibility-only | Verify the pending demo payment through ERP Public Payments and confirm receipt/accounting posting |
