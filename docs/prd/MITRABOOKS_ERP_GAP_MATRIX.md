...
| 2026-05-22 | MandirMitra sponsorship accounting | Superseded by 2026-07-13 local hardening | In-kind support now waits for maker-checker valuation approval and inventory-enabled consumables create append-only valued stock movements | Run the guarded two-user demo mutation and complete receipt/browser polish before production signoff |
| 2026-05-22 | MandirMitra receipt cancellation/reversal | Superseded by 2026-07-13 local refund hardening | Immutable receipt reversal remains; full-refund requests add maker-checker approval/rejection, payout settlement evidence, settlement-time reversal, audit, report, and CSV export | Run guarded two-user demo mutation; partial refunds and external payout execution remain deferred |
...
| MandirMitra sponsorship accounting | Local backend has pending in-kind valuation, distinct maker-checker approval, balanced/idempotent journal posting, append-only inventory receipts/issues/reversals, server-derived weighted-average issue value, negative-stock denial, fund dimensions, reporting metadata, and compensation | Full sponsorship management by event/fund with production-proven valuation and stock controls, receipt/report polish, and drill-down reporting | Run guarded two-user demo mutation; add receipt/browser polish and richer drill-down | Phase 1 |
| MandirMitra receipt cancellation/reversal | Local full-refund workflow has tenant/app-scoped request queue, distinct approval/rejection, approved-pending-settlement state, mandatory payout evidence, settlement-time linked reversal, retry-safe persistence, transition audit, report, CSV export, and guarded real-stack coverage | Production-proven refund operations with optional partial refunds and provider-authorized payout execution | Run guarded demo mutation and production review; keep partial/provider payout deferred | Phase 1 |
...
| Inventory | Implemented opt-in inventory accounting, item master, stock register, item tags on document lines, explicit weighted-average periodic valuation policy, stock issue/adjustment movement records, and closing-stock journal posting with weighted-average valuation | Inventory module with configurable valuation, stock movement audit, multi-location, and advanced item controls | Add real-stack/demo mutation, production signoff, and multi-location controls later | Phase 4 |
...
| Fixed assets | Fixed-asset register, SLM/WDV depreciation preview/posting, and local mocked-shell disposal workflow are implemented | Asset lifecycle with acquisition/disposal, depreciation, and audit reporting | Add dedicated gain/loss COA polish, real-stack/demo mutation, production asset audit reporting, and compliance review | Phase 4 |
| Dimensions | Cost-centre and project masters plus document-level tags are implemented for invoices, bills, credit notes, and debit notes; income/expense/net reports include note adjustments | Reporting dimensions across vouchers, per-line splits, and exports | Expand voucher/export coverage where missing and add production signoff | Phase 4 |
...
| Multi-client CA/bookkeeper practice | Practice tenant + per-client books and book switch implemented locally; not production-ready | Practice tenant manages many client books with staff scoped per book, approvals, and plan caps | Remaining: staff assignment per book, pricing client caps, deeper browser E2E | Phase 3 |
...
### 2026-07-13 MandirMitra fund-accounting update

This update supersedes the older sponsorship-current-state row above. New fund masters receive tenant/app-scoped accounting cost-centre dimensions. Designated donation income is tagged to the fund dimension. Fund transfers and opening balances require distinct maker and approver actors and post balanced, idempotent, reversible journals through the shared accounting service; transfer approval denies negative source-fund balances. Period fund reports derive brought-forward, opening-entry, income, expense, transfer, and closing balances from accounting-backed activity, with a dedicated as-of view. In-kind donations wait for a distinct valuation approver; inventory-enabled approvals create valued receipts, consumption uses server-derived weighted-average cost, negative stock is denied, and all issue/receipt corrections use append-only reversal movements plus journal reversal. The guarded two-user runtime mutation, receipt/browser polish, and richer drill-down remain gaps.
