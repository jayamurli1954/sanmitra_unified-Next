...
- Implemented but not production-ready: core settings backend contracts, CA practice client master/document queue/per-client books with book switch, tenant-scoped document attachments, and tenant-scoped integration/AI configuration shells.
- Planned/deferred: browser E2E depth beyond local smoke/preflight, compliance signoff, export governance, Data Health Score, advanced inventory depth, CA staff assignment per client book, and practice pricing client caps.
...
| Phase 2C | 2026-06-22 to 2026-06-30 | CA practice onboarding: client master, multi-company dashboard, client access controls, compliance tracking, and work assignment | Implemented locally: practice tenant client records, per-client `accounting_entity_id` books with CoA init, book switch via `X-Accounting-Entity-ID`, document queues; staff assignment per book and pricing caps remain later |
