...
| `office_ai` | Off | Thin AI layer; enable per tenant; gate sub-features `office_ai.tasks` / `office_ai.email` / `office_ai.brief` / `office_ai.calendar` / `office_ai.meeting_notes` / `office_ai.notifications`; opt-in `office_ai.writeback` (ADR-008) and `office_ai.workflows` (ADR-009), both default off; policy evaluation via ADR-012 before confirm/execute; opt-in `office_ai.mis` + import/export/`live_mitrabooks`/pack flags per [ADR-014](../adr/ADR-014-officemitra-ca-analysis-pack.md) (Accepted — default off; live Path A uses report services only); opt-in `office_ai.review` / `office_ai.review.working_papers` / `office_ai.review.notes` per [ADR-015](../adr/ADR-015-officemitra-review-workspaces.md) (Accepted — default off; no new Mitra brands; does not enable live MitraBooks ERP); opt-in `office_ai.documents` per [ADR-016](../adr/ADR-016-officemitra-documents-package.md) (Accepted — default off; staff CA queue read + Review note links; no client portal); opt-in `office_ai.documents.requests` per [ADR-017](../adr/ADR-017-officemitra-missing-document-requests.md) (Accepted — default off; requires parent Documents; staff gap report + OfficeMitra task; no client email) |
