...
| Frontends | Product frontend experiences are being consolidated for the four unified brands | Two deployable frontend experiences: MitraBooks Unified ERP and LegalMitra; OfficeMitra AI ships as an ERP shell panel (MVP), optional standalone later | Define API contract for menus/modules before merging UI; keep OfficeMitra thin and connector-based |
| OfficeMitra AI | Phases 1–3 implemented (`office_ai`): tasks/email/brief, calendar/notes/notifications, live Legal/Gruha/Mandir read connectors; ADRs 001–007 | Staging smoke for Phase 3; Phase 4 write-back needs new ADR; optional standalone shell later | Run Phase 3 checklist; keep connectors read-only |
