...
| OfficeMitra AI | Phases 1–6 + ADR-012 implemented; hosted foundation staging smoke **PASS** 2026-08-12 on `demo-mitrabooks-business` (incl. soft-fail Phase 4); ADR-008/009/012/014/015/016 Accepted; ADR-014 step 5 attributed narrative + citation UI **Done**; ADR-010–011 Proposed; ADR-013 Future; Review / working papers / review notes implemented behind `office_ai.review*` (**default off**, no live ERP change). Documents package implemented behind `office_ai.documents` (**default off**; staff CA queue via connector). Demo smoke seeds: `scripts/seed_review_demo.py` and `scripts/seed_documents_demo.py` on `demo-mfg-mis` only | Remaining ADR-014: live MitraBooks MIS reads; demo CA/MIS staging smoke. Review/Documents operator signoff on `demo-mfg-mis` still open | Enable Review/Documents only on `demo-mfg-mis`; do not turn on `office_ai.review*` or `office_ai.documents` for `demo-mitrabooks-business` or production ERP tenants |
