...
| **7a** | CA Analysis Pack | Excel template → MIS facts → metric pack → attributed narrative → reconcile → export (Excel/PDF/PPT); live MitraBooks report reads | **Accepted** ([ADR-014](../adr/ADR-014-officemitra-ca-analysis-pack.md)) — flags, fact store, Excel import, narrative+citation UI, reconcile, Excel/PDF/PPT export, and live Path A ingest behind `office_ai.mis.live_mitrabooks` (default off); plan [`OFFICEMITRA_ADR014_LIVE_MITRABOOKS_IMPLEMENTATION_PLAN.md`](OFFICEMITRA_ADR014_LIVE_MITRABOOKS_IMPLEMENTATION_PLAN.md); operator smoke **PASS** 2026-09-13 on `demo-mfg-mis` ([`OFFICEMITRA_MIS_LIVE_SMOKE_CHECKLIST.md`](../operations/OFFICEMITRA_MIS_LIVE_SMOKE_CHECKLIST.md)); Zoho/Tally deferred |
