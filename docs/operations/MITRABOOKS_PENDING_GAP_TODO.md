...
- [~] Banking/reconciliation browser/API E2E for CSV import, matching, reversal, reconciliation summary, bank book, and cash book polish. Local API and mocked-shell browser coverage is closed for CSV import/dedupe, matching, BRS summary, unmatch, and bank-only voucher posting from imported statement lines; guarded real-stack demo mutation now covers bank ledger voucher posting, statement CSV upload, BRS suggestion, explicit match, soft unmatch/reversal, bank-only charge classification, bank-charge voucher posting, and voucher reversal cleanup. Bank/cash book polish, live bank feed policy, and production banking signoff remain open.
