...
- [~] Dimensions/tagging coverage across vouchers, invoices, bills, notes, reports, and exports. Document-level tags and reports are now covered for invoices, bills, credit notes, debit notes, and typed vouchers with local API and mocked-shell browser coverage; dimension CSV/XLSX/PDF export route hardening is locally covered. Per-line dimensions, real-stack/demo mutation, and production signoff remain open.
