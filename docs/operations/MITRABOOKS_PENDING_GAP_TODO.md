...
- [~] Track 0 staging credentials remain **OPS-only** and are not code-fixable in this repository. Required ops runbook before rerun: rotate/reset demo admin password in secret manager -> redeploy staging with updated secret set -> reseed `demo-mitrabooks-business` user/password mapping -> rerun guarded staging mutation and attach evidence. Keep this row open until ops confirms credential parity.
...
| ID | Item | PR | Status |
| --- | --- | --- | --- |
| SEC-P0-00 | `[CRITICAL-ACCOUNTING]` Cross-store posting boundary: PostgreSQL ledger commit before Mongo source write can orphan posted journals (`post_journal_entry`, payroll, bulk import, and similar Mongo+ledger flows). Compensation must be explicit per flow with failure tests. | — | ~~[x]~~ Payroll/bulk-import compensation remained covered (`tests/test_hr_payroll_run.py`, `tests/test_bulk_import.py`) and Gruha maintenance compensation remained covered (`app/modules/housing/service.py`, `tests/test_gruhamitra_security_isolation.py`). Remaining high-risk Mongo-after-PG callers in `app/modules/mitrabooks_compat/router.py` (`create_transaction`, `post_transaction`, `reverse_transaction`) now auto-reverse posted journals on Mongo persistence failure; failure-boundary tests added in `tests/test_mitrabooks_compat_posting_guardrails.py`. Non-listed callers were re-reviewed as either PostgreSQL-only posting paths or Mongo-first with rollback-on-post-failure ordering. |
...
| SEC-P1-04 | Cap `housing_compat` journal attachment upload size; align with business/legal upload limits. | PR11 | ~~[x]~~ `app/modules/housing_compat/router.py` now enforces chunked size-limited reads for journal attachments using `HOUSING_JOURNAL_ATTACHMENT_MAX_UPLOAD_MB` (default 10 MB), aligned with business/legal limit behavior; coverage in `tests/test_housing_compat_upload_limits.py`. |
| SEC-P1-05 | Reject invalid `X-App-Key` values instead of coercing to default app (`resolve_app_key`). | — | ~~[x]~~ `app/core/tenants/context.py::resolve_app_key` now raises `InvalidAppKeyError` for unknown app keys (fallback only when header is empty/missing). `app/main.py` maps this to HTTP 400 uniformly. Coverage: `tests/integration/test_app_key_aliases.py::test_resolve_app_key_rejects_unknown_key`, `tests/test_auth_tenant_policy.py::test_get_current_user_rejects_invalid_app_key_header`. |
...
| SEC-P1-07 | Strengthen password policy (length/complexity) on register, activate, reset, and change-password. | PR11 | ~~[x]~~ Shared validator `app/core/auth/password_policy.py` enforces min length 10 plus upper/lower/number/special and is now wired into auth register/activate/reset/change-password paths in `app/core/auth/router.py` and legacy register in `app/api/legacy_alias_router.py`; coverage added in `tests/test_auth_registration_policy.py`. |
...
| SEC-P1-09 | Add audit events for super-admin `X-Tenant-ID` / `X-App-Key` override usage. | — | ~~[x]~~ `app/core/auth/dependencies.py` now writes `super_admin_context_override_used` audit events (best-effort, no token/secret logging) whenever super-admin tenant/app header overrides are used. Coverage: `tests/test_auth_tenant_policy.py::test_get_current_user_logs_super_admin_override_usage`. |
| SEC-P1-10 | Disable OpenAPI UI (`/docs`, `/redoc`) in production. | PR11 | ~~[x]~~ `app/main.py` now disables `/docs`, `/redoc`, and `/openapi.json` when `ENVIRONMENT` is `production`/`prod`; environment gating logic covered in `tests/test_auth_registration_policy.py::test_openapi_surface_is_disabled_only_in_production`. |
| SEC-P1-11 | CA invite preview/accept endpoints: add rate limits; keep response free of secrets. | — | ~~[x]~~ `app/modules/business/router.py` now applies SlowAPI limits on invite preview/accept (`20/min`, `10/min`), and accept response returns only `{ok,user_id,role}`. `app/modules/business/ca_access.py` preview drops `tenant_id`; accept service returns minimal payload (no email/token/password data). Coverage: `tests/test_rate_limit_wiring.py::test_ca_invite_rate_limits_are_wired`, `tests/test_ca_access.py::test_accept_route_accepts_text_plain_json`. |

### P2 — hardening and maintainability

- [~] Frontend: move access tokens off `localStorage` or tighten CSP; systematic `innerHTML` audit (`PR12`). Incremental pass complete: CA invite accept flow no longer depends on tenant id in public preview and tests were updated (`frontend/e2e/ca-invite-accept.spec.js`). Full auth-storage migration and strict per-shell CSP rollout remain open due cross-shell compatibility and deployment coordination risk.
...
- ~~[x] Mongo failure after ledger commit triggers documented compensation/reversal per flow.~~
