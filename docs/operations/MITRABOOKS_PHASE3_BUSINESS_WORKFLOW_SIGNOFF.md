...
| Opening Balance real-stack demo mutation | Guarded destructive demo-tenant browser/API path for opening-balance CSV preview, party-wise debtor/creditor resolution, Opening Balance Equity balancing line, admin post with duplicate override, export evidence, and generic journal reversal cleanup | Added on 2026-07-07; execution remains opt-in through the destructive demo gate |
...
| Year-End Close real-stack demo mutation | Guarded destructive demo-tenant browser/API path for dynamic FY seed activity, year-end preview, Retained Earnings movement, admin close, already-closed guard, generic journal reversal cleanup, and seed-voucher reversal | Added on 2026-07-07; execution remains opt-in through the destructive demo gate |
...
| Inventory real-stack demo mutation | Guarded destructive demo-tenant browser/API path on a run-specific accounting entity for inventory settings enablement, policy check, item create, stock adjustment/issue, stock register valuation, closing-stock journal posting, generic journal reversal cleanup, and item deactivation | Added on 2026-07-07; execution remains opt-in through the destructive demo gate |
...
| Banking/Reconciliation real-stack demo mutation | Guarded destructive demo-tenant browser/API path for bank ledger voucher posting, statement CSV upload, BRS suggestion, explicit match, BRS matched summary, soft unmatch/reversal, bank-only charge classification, bank-charge voucher posting, and voucher reversal cleanup | Added on 2026-07-07; execution remains opt-in through the destructive demo gate |
| Banking books and feed policy hardening | Backend and local shell coverage for tenant/app/entity-scoped bank/cash book period reporting, opening/receipt/payment/closing totals, account-level running balances, and documented live bank-feed policy prohibiting auto-posting and frontend credential exposure | Passed on 2026-07-07 |
| MitraBooks auth/PWA local hardening | Backend test for MitraBooks-specific reset email target, login-shell forgot-password request and reset-link completion UX, and shared PWA install prompts for Android/native and iPhone/iPad home-screen flows | Passed on 2026-07-08 |
...
| Fixed Assets real-stack demo mutation | Guarded destructive demo-tenant browser/API path on a run-specific accounting entity for fixed-asset register creation, depreciation preview/posting, balanced depreciation journal evidence, disposal journal posting, disposed-register evidence, and cross-entity denial | Added on 2026-07-08; execution remains opt-in through the destructive demo gate |
...
| Dimensions real-stack demo mutation | Guarded destructive demo-tenant browser/API path on a run-specific accounting entity for cost-centre/project masters, tagged sales invoice, purchase bill, credit note, and debit note postings, cost-centre/project report evidence, governed JSON export evidence, cross-entity denial, and reversal cleanup | Added on 2026-07-10; execution remains opt-in through the destructive demo gate |
| Branch consolidated reporting real-stack demo mutation | Guarded destructive demo-tenant browser/API path on a run-specific accounting entity for branch admin settings, cost-centre mapping, mapped invoice/bill P&L, unassigned untagged/unmapped evidence, cross-entity denial, and posted-document reversal cleanup | Added on 2026-07-11; execution remains opt-in through the destructive demo gate |
...
| Tenant-scoped document upload inbox real-stack/demo mutation | Guarded destructive demo-tenant browser/API path for CA client/document metadata creation, attachment upload/list/download, cross-book denial, manual review advancement, upload/download/review audit evidence, and OCR/auto-post disabled controls | Added on 2026-07-08; execution remains opt-in through the destructive demo gate |
| MIS/Data Health real-stack demo validation | Guarded destructive demo-tenant browser/API path on a run-specific accounting entity for posted invoice/bill source evidence, MIS KPI top-party/working-capital/overdue contracts, deterministic Financial Health KPI evidence, Data Health missing-GSTIN/unposted-draft/stale-reconciliation/overdue issue queue evidence, cross-entity denial, and posted-document reversal cleanup | Added on 2026-07-10; execution remains opt-in through the destructive demo gate |
...
### 2026-07-05 - Credit/Debit Note Line Dimensions Local Hardening
...
- PASS: focused dimension report and business route-contract tests.
- PASS: mandatory frontend preflight, including full pytest, global Playwright smoke, MitraBooks shell smoke, and CA invite smoke.
- ADDED: credit note and debit note line items carry `cost_centre_id` and `project_id` through schema, service persistence, and form payloads.
- ADDED: note create services validate line-level dimension references in the same tenant/app/entity scope as header dimensions.
...
### 2026-07-06 - Keyboard-First Business Document Entry Parity Local Hardening
...
- PASS: focused frontend local API shortcut contract tests.
- PASS: mandatory frontend preflight, including full pytest, global Playwright smoke, MitraBooks shell smoke, and CA invite smoke.
- ADDED: sales invoice, purchase bill, credit note, and debit note list views expose create-form keyboard shortcuts.
- ADDED: create forms focus the first party selector after keyboard open.
...
- Opening Balance local/demo coverage now includes mocked-shell workflow proof plus guarded real-stack preview/post/export/reversal cleanup; hosted production operator maker-checker review remains open.
- Year-End Close local/demo coverage now includes mocked-shell workflow proof plus guarded real-stack dynamic-FY preview/close/already-closed/reversal cleanup; hosted production operator maker-checker review remains open.
...
- Inventory local/demo coverage now includes item/register/closing-stock posting, explicit weighted-average periodic valuation policy, stock issue/adjustment records, guarded real-stack run-specific entity setup, closing-stock reversal cleanup, and item deactivation; multi-location controls and production inventory signoff remain open.
- Banking/reconciliation local/demo coverage now includes CSV import/dedupe, tenant/app/entity-scoped BRS assembly, exact amount/side match validation, soft unmatch/reversal, local shell import/match/BRS/unmatch UX, bank-only voucher posting from imported statement lines, bank/cash book period reporting, live bank-feed policy, and guarded real-stack bank voucher -> statement upload -> BRS suggestion -> match -> unmatch -> bank-charge voucher posting -> voucher reversal cleanup. Production banking signoff remains open.
- Fixed assets local/demo coverage now includes local API plus mocked-shell register/depreciation/disposal coverage and guarded real-stack run-specific entity setup, fixed-asset register creation, depreciation preview/posting, balanced depreciation journal evidence, disposal journal posting, disposed-register evidence, and cross-entity denial. Dedicated disposal gain/loss account-code polish, production asset audit reporting, and compliance signoff remain open.
- Dimensions local/demo coverage now includes sales invoice, purchase bill, credit note, and debit note line tagging plus document/header-level invoice, bill, note, voucher, report, export coverage locally, and guarded real-stack run-specific entity setup, cost-centre/project masters, tagged invoice/bill/credit-note/debit-note postings, cost-centre/project report evidence, governed JSON export evidence, cross-entity denial, and reversal cleanup. Production dimensions signoff remains open.
- Branch consolidated reporting local/demo coverage now includes local API plus mocked-shell branch rollup coverage and guarded real-stack run-specific entity setup, branch admin cost-centre mapping, mapped posted invoice/bill P&L, unassigned untagged/unmapped evidence, cross-entity denial, and reversal cleanup. Branch selector UX across document entry, export route, and production multi-branch signoff remain open.
- Tenant-scoped document upload inbox local/demo coverage now includes backend plus mocked-shell client/book metadata, attachment, audit, review evidence, object-storage/OCR provider policy, and guarded real-stack CA client/document creation, upload/list/download, cross-book denial, manual review, audit evidence, and OCR/auto-post disabled controls. Provider configuration, malware/file-type hardening, and production signoff remain open.
- MIS KPI contracts local/demo coverage now includes deterministic local backend assembly, mocked-shell rendering, and guarded real-stack run-specific posted invoice/bill source evidence for top-party, working-capital, overdue, and embedded deterministic Financial Health contracts. Production report signoff and AI MIS narration remain open.
- Data Health Score local/demo coverage now includes deterministic local backend rules, issue-list normalization, mocked-shell rendering/routing, and guarded real-stack missing-GSTIN, unposted-draft, stale-reconciliation, overdue-exposure issue queue evidence plus cross-entity denial. Duplicate-invoice real-stack evidence, persisted assignee/status workflow, and production signoff remain open.
- Export governance local/demo coverage now includes local role checks, audit metadata, governed response headers, JSON format support for generic report-style exports, and guarded real-stack dimension JSON, opening-balance CSV, trial-balance JSON, sales-invoice PDF, Tally XML header/body/audit evidence. Production audit-retention policy and wider GST/e-invoice JSON download governance remain open.
...
- Keyboard-first voucher/business-entry polish still needs production operator UX signoff; the current gates close local voucher-entry plus sales invoice, purchase bill, credit note, and debit note keyboard flow coverage.
