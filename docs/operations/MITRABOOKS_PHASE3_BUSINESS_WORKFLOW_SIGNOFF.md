...
- Tenant-scoped document upload inbox still needs real-stack/demo mutation, provider configuration, malware/file-type hardening, and production signoff; the current gate closes local backend plus mocked-shell client/book metadata, attachment, audit, review evidence, and the object-storage/OCR provider policy in `docs/operations/MITRABOOKS_DOCUMENT_UPLOAD_OCR_POLICY.md`.
