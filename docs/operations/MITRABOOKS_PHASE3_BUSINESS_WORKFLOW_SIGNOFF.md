...
| Banking/Reconciliation real-stack demo mutation | Guarded destructive demo-tenant browser/API path for bank ledger voucher posting, statement CSV upload, BRS suggestion, explicit match, BRS matched summary, soft unmatch/reversal, bank-only charge classification, and voucher reversal cleanup | Added on 2026-07-07; execution remains opt-in through the destructive demo gate |
...
- Banking/reconciliation local/demo coverage now includes CSV import/dedupe, tenant/app/entity-scoped BRS assembly, exact amount/side match validation, soft unmatch/reversal, local shell import/match/BRS/unmatch UX, and guarded real-stack bank voucher -> statement upload -> BRS suggestion -> match -> unmatch -> voucher reversal cleanup. Bank-only voucher posting from imported statement lines, bank book/cash book production polish, live bank feed policy, and production banking signoff remain open.
