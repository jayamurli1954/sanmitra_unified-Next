#!/usr/bin/env python3
"""Wire payment-allocation module into app.js; bump cache v60→v61; update baseline."""
...
def main() -> None:
    ...
    if "modules/workspaces/payment-allocation.js" in app:
        print("Already wired")
        return

    marker = 'from "./modules/workspaces/opening-yearend.js";\n'
    if marker not in app:
        raise SystemExit("opening-yearend import marker not found")
    app = app.replace(marker, marker + "\n" + IMPORT, 1)

    idx = app.find("initOpeningYearEnd({")
    if idx < 0:
        raise SystemExit("initOpeningYearEnd not found")
    end = app.find("});\n", idx)
    if end < 0:
        raise SystemExit("initOpeningYearEnd close not found")
    end += len("});\n")
    app = app[:end] + "\n" + INIT + app[end:]
    ...
    html2, n = re.subn(
        r"app\.js\?v=mitrabooks-erp-v60",
        "app.js?v=mitrabooks-erp-v61",
        html,
        count=1,
    )
    if n != 1:
    ...
    print(f"Wired payment-allocation; cache v61; app.js={app_lines}")
