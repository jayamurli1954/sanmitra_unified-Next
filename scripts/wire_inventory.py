#!/usr/bin/env python3
"""Wire inventory module into app.js; bump cache v52→v53; update baseline."""
...
def main() -> None:
    ...
    if "modules/workspaces/inventory.js" in app:
        print("Already wired")
        return

    marker = 'from "./modules/workspaces/fixed-assets.js";\n'
    if marker not in app:
        raise SystemExit("fixed-assets import marker not found")
    app = app.replace(marker, marker + "\n" + IMPORT, 1)

    idx = app.find("initFixedAssets({")
    if idx < 0:
        raise SystemExit("initFixedAssets not found")
    end = app.find("});\n", idx)
    if end < 0:
        raise SystemExit("initFixedAssets close not found")
    end += len("});\n")
    app = app[:end] + "\n" + INIT + app[end:]
    ...
    html2, n = re.subn(
        r"app\.js\?v=mitrabooks-erp-v52",
        "app.js?v=mitrabooks-erp-v53",
        html,
        count=1,
    )
    if n != 1:
    ...
    print(f"Wired inventory; cache v53; app.js={app_lines}")
