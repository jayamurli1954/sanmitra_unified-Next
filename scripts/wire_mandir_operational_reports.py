#!/usr/bin/env python3
"""Wire mandir-operational-reports into app.js; bump cache v84→v85; update baseline."""
...
def main() -> None:
    ...
    if 'from "./modules/workspaces/mandir-operational-reports.js"' in app:
        print("Already wired")
        return

    marker = 'from "./modules/workspaces/dashboard-primitives.js";\n'
    if marker not in app:
        raise SystemExit("dashboard-primitives import marker not found")
    app = app.replace(marker, marker + "\n" + IMPORT, 1)

    idx = app.find("initDashboardPrimitives({")
    if idx < 0:
        raise SystemExit("initDashboardPrimitives not found")
    app = app[:idx] + INIT + "\n" + app[idx:]
    ...
    html2, n = re.subn(
        r"app\.js\?v=mitrabooks-erp-v84",
        "app.js?v=mitrabooks-erp-v85",
        html,
        count=1,
    )
    if n != 1:
    ...
    print(f"Wired mandir-operational-reports; app.js={app_lines}; cache=v85")
