#!/usr/bin/env python3
"""Wire TDS module into app.js; bump cache v56→v57; update baseline."""
...
def main() -> None:
    ...
    if "modules/workspaces/tds.js" in app:
        print("Already wired")
        return

    marker = 'from "./modules/workspaces/einvoice.js";\n'
    if marker not in app:
        raise SystemExit("einvoice import marker not found")
    app = app.replace(marker, marker + "\n" + IMPORT, 1)

    idx = app.find("initEinvoice({")
    if idx < 0:
        raise SystemExit("initEinvoice not found")
    end = app.find("});\n", idx)
    if end < 0:
        raise SystemExit("initEinvoice close not found")
    end += len("});\n")
    app = app[:end] + "\n" + INIT + app[end:]
    ...
    html2, n = re.subn(
        r"app\.js\?v=mitrabooks-erp-v56",
        "app.js?v=mitrabooks-erp-v57",
        html,
        count=1,
    )
    if n != 1:
    ...
    print(f"Wired tds; cache v57; app.js={app_lines}")
