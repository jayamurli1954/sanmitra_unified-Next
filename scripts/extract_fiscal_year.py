#!/usr/bin/env python3
"""Extract Indian FY helpers into modules/workspaces/fiscal-year.js (Phase 3 seam 46)."""
...
OUT = ROOT / "frontend/mitrabooks-erp/modules/workspaces/fiscal-year.js"
...
def main() -> None:
    if OUT.exists() and "export function currentFinancialYear" in OUT.read_text(encoding="utf-8"):
    ...
    text = "".join(lines)
    text = re.sub(
        r"(?m)^// Current Indian financial year as \"YYYY-YY\" \(FY starts April\)\.\n+",
        "",
        text,
        count=1,
    )
    text = re.sub(
        r"(?m)^// Current Indian-FY quarter as \"YYYY-Q\[1-4\]\" \(FY starts April; Q1 = Apr-Jun\)\.\n+",
        "",
        text,
        count=1,
    )
    text = re.sub(
        r"(?m)^// A handful of recent FY quarters for the CMP-08 picker\.\n+",
        "",
        text,
        count=1,
    )
