"""Extract debit notes workspace from app.js into modules/documents/debit-notes.js."""
...
OUT = ROOT / "frontend" / "mitrabooks-erp" / "modules" / "documents" / "debit-notes.js"
...
REPLACEMENTS = [
    ("DN_REASONS", "debitUi.reasons"),
    ("debitNoteView", "debitUi.view"),
    ("lastDebitNotes", "debitUi.notes"),
    ("lastDebitNoteDetail", "debitUi.detail"),
    ("dnLineSeq", "debitUi.lineSeq"),
    ("dnFormLines", "debitUi.formLines"),
    ("dnFormHeader", "debitUi.header"),
    ("dnReverseOpen", "debitUi.reverseOpen"),
...
def main() -> None:
    ...
    start = next(i for i, l in enumerate(lines) if "Business Module: Debit Notes" in l)
    end = next(i for i, l in enumerate(lines) if "Business Module: Typed Vouchers" in l)
    block_lines = lines[start:end]

    first_fn = next(
        i for i, l in enumerate(block_lines) if l.startswith("function ") or l.startswith("async function ")
    )
    ...
    for line in body.splitlines(True):
        ...
        if (
            stripped.startswith("// ══")
            or stripped.startswith("// SECTION: DEBIT")
            or stripped.startswith("// API")
            or stripped.startswith("// NOTE  : loadDebitNotes")
        ):
            continue
        cleaned.append(line)
    body2 = "".join(cleaned)
    ...
    export_names = [
        ...
        "loadDebitNotes",
        "resolveDebitNoteSourceBill",
        "debitNoteSourceBillOptions",
        "syncDnFormFromDom",
        ...
        "dnReasonLabel",
        "renderDnListTable",
        "renderDebitNoteCreateForm",
        "renderDebitNoteDetail",
