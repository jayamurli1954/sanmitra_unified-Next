#!/usr/bin/env python3
"""Extract shared render/format helpers (Phase 3 seam 63) — leaf module, no init."""
from __future__ import annotations

import json
import re
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
APP = ROOT / "frontend/mitrabooks-erp/app.js"
OUT = ROOT / "frontend/mitrabooks-erp/modules/workspaces/shared-render-utils.js"
INDEX = ROOT / "frontend/mitrabooks-erp/index.html"
BASELINE = ROOT / "scripts/file_size_baseline.json"

HEADER = '''\
// ====================================================================
// SECTION: SHARED RENDER / FORMAT UTILS
// Extracted from app.js per docs/operations/LARGE_FILE_MODULARIZATION_PLAN.md.
// Pure move: logic unchanged. Leaf module — no init; import getConfiguredApiBaseUrl.
// ====================================================================

import { getConfiguredApiBaseUrl } from "../../../shared/api-client.js";

'''

EXPORT_FUNCS = [
    "buildFrontendApiUrl",
    "resultPayload",
    "resultRows",
    "statusDetailText",
    "renderStatusBlock",
    "currentBillingPeriodQuery",
    "renderSimpleTable",
    "escapeHtml",
    "formatCountMap",
    "renderPlatformTable",
    "renderPendingApprovalsTable",
    "mandirPublicPaymentPageUrl",
    "renderMandirOperationResult",
    "delay",
    "formatCurrency",
    "formatCountLabel",
]

# Keep setLoginStatus in app.js (loginStatus DOM ref).

IMPORT = '''\
import {
  buildFrontendApiUrl,
  resultPayload,
  resultRows,
  statusDetailText,
  renderStatusBlock,
  currentBillingPeriodQuery,
  renderSimpleTable,
  escapeHtml,
  formatCountMap,
  renderPlatformTable,
  renderPendingApprovalsTable,
  mandirPublicPaymentPageUrl,
  renderMandirOperationResult,
  delay,
  formatCurrency,
  formatCountLabel,
} from "./modules/workspaces/shared-render-utils.js";
'''


def find_fn_block(lines: list[str], name: str) -> tuple[int, int]:
    start = next(i for i, l in enumerate(lines) if re.match(rf"^(async )?function {re.escape(name)}\b", l))
    depth = 0
    started = False
    end = start
    for i in range(start, len(lines)):
        depth += lines[i].count("{") - lines[i].count("}")
        started = True
        if started and depth <= 0:
            end = i + 1
            break
    else:
        raise SystemExit(f"unterminated {name}")
    while end < len(lines) and lines[end].strip() == "":
        end += 1
    return start, end


def main() -> None:
    if OUT.exists() and "export function escapeHtml" in OUT.read_text(encoding="utf-8"):
        print("Already extracted")
        return

    lines = APP.read_text(encoding="utf-8").splitlines(keepends=True)
    spans = []
    for name in EXPORT_FUNCS:
        start, end = find_fn_block(lines, name)
        spans.append((start, end, name))
    spans.sort(key=lambda s: s[0], reverse=True)
    chunks: dict[str, str] = {}
    for start, end, name in spans:
        chunks[name] = "".join(lines[start:end])
        del lines[start:end]

    block = "".join(chunks[name] for name in EXPORT_FUNCS)
    for name in EXPORT_FUNCS:
        block = re.sub(
            rf"(?m)^(async )?function {name}\b",
            rf"export \1function {name}",
            block,
            count=1,
        )
    block = block.replace("export export ", "export ")

    module = HEADER + block
    if not module.endswith("\n"):
        module += "\n"
    OUT.write_text(module, encoding="utf-8", newline="\n")

    app = "".join(lines)

    # Placeholder comments where the shared-utils cluster lived
    app = app.replace(
        "// Business executive dashboard lives in modules/workspaces/executive-dashboard.js\n\n",
        "// Business executive dashboard lives in modules/workspaces/executive-dashboard.js\n"
        "// Shared render/format utils live in modules/workspaces/shared-render-utils.js\n\n",
        1,
    )
    app = re.sub(
        r"(?m)^// Shared utilities \(escapeHtml, formatters, auth status\) remain below\.\n"
        r"^// Nav workspace mapping \+ sync live in modules/workspaces/navigation-shell\.js\n\n",
        "// Nav workspace mapping + sync live in modules/workspaces/navigation-shell.js\n\n",
        app,
        count=1,
    )
    app = re.sub(
        r"(?m)^// ═+\n\n// ═+\n"
        r"// Shared money/count formatters \(used across workspaces\)\n"
        r"// ═+\n\n",
        "",
        app,
        count=1,
    )

    # Insert import after experience-config or early after other leaf imports
    marker = 'from "./modules/workspaces/experience-config.js";\n'
    if marker not in app:
        raise SystemExit("experience-config import marker not found")
    app = app.replace(marker, marker + "\n" + IMPORT, 1)

    APP.write_text(app, encoding="utf-8", newline="\n")

    html = INDEX.read_text(encoding="utf-8")
    html2, n = re.subn(r"app\.js\?v=mitrabooks-erp-v100", "app.js?v=mitrabooks-erp-v101", html, count=1)
    if n != 1:
        raise SystemExit(f"cache bump failed n={n}")
    INDEX.write_text(html2, encoding="utf-8", newline="\n")

    baseline = json.loads(BASELINE.read_text(encoding="utf-8"))
    app_lines = len(APP.read_text(encoding="utf-8").splitlines())
    baseline["frontend/mitrabooks-erp/app.js"] = app_lines
    BASELINE.write_text(json.dumps(baseline, indent=2) + "\n", encoding="utf-8", newline="\n")

    print(f"Wrote {OUT.relative_to(ROOT)} ({len(module.splitlines())} lines)")
    print(f"Wired shared-render-utils; app.js={app_lines}; cache=v101")


if __name__ == "__main__":
    main()
