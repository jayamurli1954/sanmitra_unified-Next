#!/usr/bin/env python3
"""Wire dashboard-primitives into app.js; bump cache v83→v84; update baseline."""
...
def main() -> None:
    ...
    if 'from "./modules/workspaces/dashboard-primitives.js"' in app:
        print("Already wired")
        return

    marker = 'from "./modules/workspaces/fiscal-year.js";\n'
    if marker not in app:
        raise SystemExit("fiscal-year import marker not found")
    app = app.replace(marker, marker + "\n" + IMPORT, 1)

    idx = app.find("initBusinessListFilters({")
    if idx < 0:
        raise SystemExit("initBusinessListFilters not found")
    app = app[:idx] + INIT + "\n" + app[idx:]
    ...
    html2, n = re.subn(
        r"app\.js\?v=mitrabooks-erp-v83",
        "app.js?v=mitrabooks-erp-v84",
        html,
        count=1,
    )
    if n != 1:
    ...
    print(f"Wired dashboard-primitives; app.js={app_lines}; cache=v84")
