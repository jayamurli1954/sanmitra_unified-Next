#!/usr/bin/env python3
"""Extract business attachment helpers into modules/workspaces/attachments.js (Phase 3 seam 43)."""
...
OUT = ROOT / "frontend/mitrabooks-erp/modules/workspaces/attachments.js"
...
def extract_blocks(lines: list[str], names: list[str]) -> tuple[str, list[str]]:
    ...
    for name in names:
        # Prefer exact function name match at start
        start = next(
            i
            for i, l in enumerate(lines)
            if re.match(rf"^(async )?function {re.escape(name)}\b", l.lstrip())
            or (l.lstrip().startswith(("function ", "async function ")) and f"function {name}" in l)
        )
        # Use brace matching from start
        depth = 0
        started = False
        end = start
        for i in range(start, len(lines)):
...
def main() -> None:
    if OUT.exists() and "export function businessAttachmentPath" in OUT.read_text(encoding="utf-8"):
    ...
    # Use module-local apiRequest import instead of shell global (app had apiRequest in scope).
    # listBusinessAttachments already calls apiRequest(...) — keep as imported binding.
